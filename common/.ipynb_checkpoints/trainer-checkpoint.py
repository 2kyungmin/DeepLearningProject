...
class RnnlmTrainer:
    ...
    def fit(self, xs, ts, max_epoch=10, batch_size=20, time_size=35, max_grad=None, eval_interval=20):
        data_size = 0
        for x in xs: data_size += len(x)
        # batch_size: 한 번에 학습할 시계열 샘플 개수
        # time_size: RNN이 한 번에 펼쳐서 보는 time step 길이, BPTT의 길이
        # 한 epoch 동안 몇번의 미니배치를 뽑을 수 있는지 계산
        max_iters = data_size // (batch_size * time_size)
        self.time_idx = 0
        # 에폭 동안 측정된 퍼플렉서티를 저장할 리스트
        self.ppl_list = []
        # 몇 번 반복마다 퍼플렉서티를 출력할지
        self.eval_interval = eval_interval
        model, optimizer = self.model, self.optimizer
        total_loss = 0    # 퍼플렉서티 계산을 위한 누적 손실
